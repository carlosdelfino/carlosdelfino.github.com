...
def gerar_index(certificados: list[dict]) -> None:
    ...
    linhas.append(
        '    content.innerHTML = \'<object data="\' + url + \'" '
        'type="application/pdf" width="100%" height="100%">'
        '<p class="cert-modal-fallback">'
        'Seu navegador não suporta visualização de PDF embutido. '
        '<a href="\' + url + \'" target="_blank">'
        'Clique aqui para abrir o certificado</a></p></object>\';'
    )
    linhas.append("  } else {")
    linhas.append(
    ...
    linhas.append("}")
    linhas.append("// Move o modal para o body para evitar que")
    linhas.append("// transforms de elementos pai quebrem position:fixed")
